  ...
  getAllProducts: async () => {
    try {
      ...
      for (const product of enrichedProducts) {
        product.promotions =
          await promotionService.getActivePromotionsForProduct(product.id);
      }
  ...
  getProductDetail: async (productId) => {
    try {
      ...
      if (cachedProduct) {
        ...
        // Add promotions
        product.promotions =
          await promotionService.getActivePromotionsForProduct(productId);
      ...
      if (cachedRelated) {
        ...
        const categoryProducts = await productsModel.some(
          "cateId",
          product.cateid
        );
        ...
        // Enrich with ratings, images, and promotions
        relatedProducts = await Promise.all(
          filteredProducts.map(async (relatedProduct) => {
            // Get product rating
            const { average } = (await reviewService.getRating(
              relatedProduct.id
            )) ?? { average: 0 };
            ...
            // Get promotions
            relatedProduct.promotions =
              await promotionService.getActivePromotionsForProduct(
                relatedProduct.id
              );
      ...
      console.error(
        `Error in getProductDetail for product ${productId}:`,
        error
      );
      ...
      try {
        ...
        console.error(
          `Fallback error in getProductDetail for product ${productId}:`,
          fallbackError
        );

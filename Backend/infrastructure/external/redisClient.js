...
client.on('ready', () => {
    ...
    console.log(`Redis client ready (connected to ${config.redis.host}:${config.redis.port}).`);
});
...
client.on('error', (err) => {
    console.error('Redis Client Error:', err.message || err);
     if (!connectionEstablished && isConnecting) {
         console.error("FATAL ERROR: Could not establish initial connection to Redis.");
         process.exit(1);
     }
...
const connectRedis = async () => {
    if (!client.isOpen && !isConnecting) {
        ...
        try {
            ...
            console.error('Failed to connect to Redis on startup:', err.message || err);
             process.exit(1);
        } finally {
             isConnecting = false;
        }

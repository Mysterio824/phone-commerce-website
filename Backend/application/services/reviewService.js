...
const reviewService = { 
    ...
    getRating: async (productId, forceUpdate = false) => {
        try {
            ...
            const average = reviews.reduce((sum, r) => sum + parseFloat(r.rating), 0) / count;
    ...
    getReviews: async (productId, page = 1, perPage = 10) => {
        try {
            ...
            const { currentPage, totalPages, startIdx, endIdx } = getPagination(page, perPage, reviews.length);

            return { currentPage, totalPages, reviews: reviews.slice(startIdx, endIdx) };
        } catch (error) {
            console.error(`Error getting reviews for product ${productId}:`, error);            
            try {
                const reviews = await reviewModel.some('productId', productId, page, perPage);
                if (!reviews || reviews.length === 0) {
                    return { currentPage: 1, totalPages: 1, reviews: [] };
                }
                const { currentPage, totalPages, startIdx, endIdx } = getPagination(page, perPage, reviews.length);
